...
async function preload() {
    ...
    async function getSupportedStableMinecraftVersions() {
        try {
            ...
            console.log(
                "[preload] Error fetching supported Minecraft versions:",
                error
            );
            return [];
        }
        ...
        try {
            ...
            return data
                ...
                .filter((version) => {
                    ...
                    return (
                        versionNumbers.length >= 2 && versionNumbers[1] >= 12
                    );
                })
                .sort((a, b) =>
            ...
            console.log(
                "[preload] Error fetching Fabric Loader versions:",
                error
            );
            return [];
        }
    ...
    try {
        ...
        const versionPromises = supportedMinecraftVersions.map(
            async (minecraftVersion) => {
                const builds = fabricLoaderVersions.map(
                    (fabricLoaderVersion) => ({
...
module.exports = {
    ...
    getLatest: async () => {
        ...
        // Get latest build for that version (first after sorting by semver)
        const sortedBuilds = versionEntry.builds
            .map((b) => b.build)
            .sort((a, b) =>
                semver.rcompare(semver.coerce(a), semver.coerce(b))
            );
